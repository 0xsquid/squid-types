export enum FeatureFlagType {
  MaintenanceMode = "maintenance",
  Bitcoin = "bitcoin",
  BitcoinCoralV2 = "bitcoinCoralV2",
  Solana = "solana",
  SolanaCoralV2 = "solanaCoralV2",
  Chainflip = "chainflip",
  Coral = "coral",
  CoralOnly = "coralOnly",
  CoralV2 = "coralV2",
  CoralV2PostHooks = "coralV2PostHooks",
  CoralV2CustomPostHooks = "coralV2CustomPostHooks",
  CoralV2PreHooks = "coralV2PreHooks",
  ChainflipMultihop = "chainflipMultihop",
  SagaRoutes = "sagaRoutes",
  BypassCompliance = "bypassCompliance",
  Ripple = "ripple",
  RippleCoralV2 = "rippleCoralV2",
  Megabridge = "megabridge",
  Stellar = "stellar",
  StellarCoralV2 = "stellarCoralV2",
  Mantra = "mantra",
  HederaPilot = "hederaPilot",
  Citrea = "citrea",
  HederaCoralV2 = "hederaCoralV2",
  HederaBlocked = "hederaBlocked",
  AddressQuoteRateLimit = "addressQuoteRateLimit",
  ExactOutputRouting = "exactOutputRouting",
  Tempo = "tempo",
  TempoCoralV2 = "tempoCoralV2",
  Kaia = "kaia",
  KaiaCoralV2 = "kaiaCoralV2",
  Robinhood = "robinhood",
  RobinhoodCoralV2 = "robinhoodCoralV2",
  Kub = "kub",
  KubCoralV2 = "kubCoralV2",
  XLayer = "xLayer",
  XLayerCoralV2 = "xLayerCoralV2",
  Unichain = "unichain",
  UnichainCoralV2 = "unichainCoralV2",
  Ink = "ink",
  InkCoralV2 = "inkCoralV2",
  FraxtalCoralV2 = "fraxtalCoralV2",
  ScrollCoralV2 = "scrollCoralV2",
  KavaCoralV2 = "kavaCoralV2",
  ImmutableZkEvmCoralV2 = "immutableZkEvmCoralV2",
  MantleCoralV2 = "mantleCoralV2",
  AddressValidation = "addressValidation",
  SquidSend = "squidSend",
  FilecoinCoralV2 = "filecoinCoralV2",
  CantonCoralV2 = "cantonCoralV2",
  CoralV2EchoStatus = "coralV2EchoStatus",
  FeatureManagerV2 = "featureManagerV2",
  Danal = "danal",
  DanalCoralV2 = "danalCoralV2",
}

export interface FeatureFlag {
  feature: FeatureFlagType;
  allowedIntegrators: string[];
  enabled: boolean;
}
